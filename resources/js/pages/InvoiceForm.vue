<template>
  <div>
    <router-link to="/invoices" class="text-sm text-slate-600 hover:text-slate-800 mb-4 inline-block">← Back to Invoices</router-link>
    <h1 class="text-2xl font-bold text-slate-800 mb-6">{{ isEdit ? 'Edit Invoice' : 'New Invoice' }}</h1>
    ...
    <form @submit.prevent="submit" class="space-y-6 max-w-4xl">
      <div class="bg-white rounded-lg border border-slate-200 p-6 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Exchange rate (to base)</label>
            <input v-model.number="form.exchange_rate" type="number" step="0.000001" min="0.000001" class="w-full rounded-lg border border-slate-300 px-3 py-2" placeholder="1" />
            <p class="text-xs text-slate-500 mt-0.5">1 {{ form.currency }} = {{ form.exchange_rate }} base currency</p>
          </div>
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Invoice date *</label>
            <input v-model="form.invoice_date" type="date" required class="w-full rounded-lg border border-slate-300 px-3 py-2" />
          </div>
          <div>
      ...
      <div class="flex gap-3">
        ...
        <router-link v-if="isEdit" to="/invoices" class="px-4 py-2 border border-slate-300 rounded-lg">Cancel</router-link>
      </div>
    </form>
  </div>
</template>
...
async function loadInvoice() {
  ...
  form.value = {
    ...
    invoice_date: inv.invoice_date?.slice(0, 10) || form.value.invoice_date,
    due_date: inv.due_date?.slice(0, 10) || form.value.due_date,
    notes: inv.notes || '',
    items: (inv.items || []).length ? inv.items.map(i => ({ description: i.description, quantity: parseFloat(i.quantity), price: parseFloat(i.price), tax: parseFloat(i.tax) || 0, discount: parseFloat(i.discount) || 0 })) : [{ description: '', quantity: 1, price: 0, tax: 0, discount: 0 }],
  };
}
