<template>
  <div>
    ...
    <div class="flex items-center justify-between gap-4 mb-6">
      <h1 class="text-2xl font-bold text-slate-800">{{ t('AP Aging') }}</h1>
...
onMounted(async () => {
  try {
    const { data: res } = await api().get('/reports/ap-aging');
    data.value = res.data;
  } catch (e) {
    console.error(e);
  } finally {
...
function downloadPdf() {
  downloadReportPdf('/reports/ap-aging/pdf', { as_of: data.value.as_of });
}
</script>
