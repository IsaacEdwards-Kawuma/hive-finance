<template>
  <div>
    <div class="flex flex-wrap justify-between items-center gap-4 mb-6">
      <h1 class="text-2xl font-bold text-slate-800">{{ t('Customers') }}</h1>
      <div class="flex gap-2">
        ...
        <button @click="openForm()" class="px-4 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-700 text-sm">{{ t('Add customer') }}</button>
      </div>
    </div>
    <div class="mb-4 flex gap-2">
      <input v-model="search" type="text" :placeholder="t('Search') + ' ' + t('Customers')" class="rounded-lg border border-slate-300 px-3 py-2 text-sm flex-1 max-w-xs" @keyup.enter="fetchList" />
      <button type="button" @click="fetchList" class="px-4 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-700 text-sm">{{ t('Run') }}</button>
    </div>
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          <tr v-for="c in customers" :key="c.id" class="hover:bg-slate-50">
            ...
            <td class="px-4 py-3 text-right"><button type="button" @click="openForm(c)" class="text-slate-600 hover:text-slate-800">{{ t('Edit') }}</button></td>
          </tr>
          <tr v-if="!customers.length">
            <td colspan="4" class="px-4 py-8 text-center text-slate-500">{{ t('No customers yet. Add one to get started.') }}</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div v-if="showForm" class="fixed inset-0 bg-black/50 flex items-center justify-center z-10 p-4" @click.self="showForm = false">
      <div class="bg-white rounded-xl shadow-xl max-w-sm w-full p-6">
        <h2 class="font-semibold text-lg mb-4">{{ editingId ? t('Edit') + ' ' + t('Customer') : t('New customer') }}</h2>
...
async function fetchList() {
  try {
    ...
    customers.value = Array.isArray(data?.data) ? data.data : (data?.data ? [data.data] : []);
  } catch (e) {
    console.error(e);
    customers.value = [];
  } finally {
...
function openForm(c = null) {
  ...
  form.value = { name: c?.name ?? '', email: c?.email ?? '', phone: c?.phone ?? '' };
  showForm.value = true;
}
...
function exportCSV() {
  ...
  const rows = customers.value.map(c => [c.name, c.email || '', c.phone || ''].map(x => '"' + String(x).replace(/"/g, '""') + '"'));
