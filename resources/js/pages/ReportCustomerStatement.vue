<template>
  <div>
    ...
    <h1 class="text-2xl font-bold text-slate-800 mb-6">Customer statement</h1>
    <div class="flex flex-wrap gap-4 items-end mb-6">
      <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">Customer</label>
        <select v-model="customerId" class="rounded-lg border border-slate-300 px-3 py-2 min-w-[200px]">
      ...
      <button v-if="customerId" type="button" @click="downloadPdf" class="px-4 py-2 bg-slate-700 text-white rounded-lg hover:bg-slate-600 text-sm">Download PDF</button>
    </div>
...
async function load() {
  ...
  run.value = true;
  try {
    const { data } = await api().get('/reports/customer-statement', { params: { customer_id: customerId.value, from: from.value, to: to.value } });
...
function downloadPdf() {
  downloadReportPdf('/reports/customer-statement/pdf', { customer_id: customerId.value, from: from.value, to: to.value });
}
